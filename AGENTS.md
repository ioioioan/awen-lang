...
An `.awen` file captures the creator's committed production intent: output, authoritative sources, stack/tool choices, requirements, preferences, exclusions, approval gates, and completion criteria.
...
2. Resolve and read its declared sources according to their roles and authority.
3. Treat the assembly sheet and `AUTHORITATIVE` sources as the committed handoff.
4. Use `CONTEXT` sources as supporting material, not as authority over committed decisions.
5. Follow declared constraints and approval gates.
6. Use ordinary domain tooling to produce the requested artifact.
7. Prefer deterministic verification where available: tests, compilers, schemas, validators, linters, render checks, or equivalent objective checks.
8. Do not change the committed build contract silently.
